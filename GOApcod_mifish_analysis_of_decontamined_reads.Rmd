...
read in sample metadata - not sure if i really need this. most metadata is in the decontaminated reads file. 
```{r}
...
join the taxon id's for the ASVs in the decontaminated read dataset 
```{r}
...
okay. just under half (93/196) of the asv's have matching ids... 
...
hmm... okay what options do i have instead of going in by hand..? 
- for the asvs with 0 diff between the species, go with the blastn id?  but what if only one of the species is within our range? 
- remove asvs with no blastn id? many have higher taxonomic ids in the insect.. 
...
i'm having trouble with rfishbase... going to just upload a species list i downloaded from fishbase for all Pacific, Northeast waters species. 
```{r}
...
should any of these still be there?? 


what asvs now only have one species match? 
```{r}
...
most of these ASVs without taxonomic ids don't have very many reads... but maybe i should look into the top few later on to see if there is a reasonable id out there... 

anyways... for now i will remove those ASVs.
```{r}
asv_w_id <- asv_w_id %>%
  filter(!is.na(taxon)) 
```
...
**most of this info is already there... not joining metadata for now. 
